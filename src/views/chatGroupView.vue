              ...
              <div class="flex flex-wrap gap-2 relative">
                ...
                >
                  <div class="flex items-center justify-between">
                    <div class="flex items-center gap-2">
                      <img 
                        v-if="selectedUser && userStore.getUserById(selectedUser)?.imgUrl" 
                        :src="userStore.getUserById(selectedUser)?.imgUrl" 
                  ...
                  >
                    ...
                    <div>
                      <div class="text-gray-900 dark:text-white">
                        {{ user.fullNameThai }} 
                        <span class="text-gray-500 text-xs" v-if="user.positon">{{ user.positon }} <span class="text-gray-500">: {{ user.department }}</span></span>
                      </div>
...
<!--                    </th>-->
                  ...
                  <tbody>
                  <tr v-for="member in currentRoom.members" :key="member.empId" class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 border-gray-200">
                    <th scope="row" class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                      <div class="flex items-center gap-2">
                        <img 
                          :src="getUserImage(member.empId)"
                          class="w-8 h-8 rounded-full object-cover"
                          alt="User profile"
                          @error="handleImageError"
                        />
                        <div class="flex items-center gap-2">
                          ...
                          <span v-if="member.role === 'bot'" class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-gray-100 text-gray-800 dark:bg-gray-700 dark:text-gray-300">
...
<!--                    </td>-->
                    ...
                    <td class="px-6 py-4">
                      <button 
                        v-if="member.role !== 'bot'"
                        @click="removeMember(member.empId)" 
                        class="text-red-600 hover:text-red-800 dark:text-red-400 dark:hover:text-red-300"
                      >
                        <Icon icon="mdi:delete" width="24" height="24" />
                      </button>
      ...
      <div v-if="showEditModal" class="fixed inset-0 bg-gray-900 bg-opacity-50 overflow-y-auto h-full w-full z-50">
        <div class="relative top-20 mx-auto p-5 border w-[600px] shadow-lg rounded-md bg-white dark:bg-gray-800">
          ...
          <form @submit.prevent="updateRoom()">
            <div class="mb-4">
              <label for="edit-name" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">ชื่อกลุ่ม</label>
              <input 
                type="text" 
                id="edit-name" 
                v-model="currentRoom.name" 
            ...
            <div class="mb-4">
              <label for="edit-description" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">คำอธิบาย</label>
              <textarea 
                id="edit-description" 
                v-model="currentRoom.description" 
                required
          ...
          <div>
            ...
            <div class="space-y-2">
              ...
              >
                ...
                <button 
                  v-if="member.id !== selectedRoom.admin"
  ...
  computed: {
    ...
    filteredUsers() {
      ...
      return this.mockUsers.filter(user => {
        const isNotMember = !this.selectedRoom?.members.some(m => m.id === user.id);
  ...
  methods: {
    ...
    async createRoom() {
      try {
        ...
        const response = await this.chatStore.createChat(this.currentRoom);

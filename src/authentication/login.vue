  ...
  /> -->
      <div
        ...
        <div class="flex justify-center items-center mb-6">
           <span class="text-xl font-semibold text-gray-500 dark:text-gray-400">เข้าสู่ระบบจัดการ 12Chat</span>
        </div>
        <div class="p-6 space-y-4 md:space-y-6 sm:p-6" style="padding-top: 0%">
          <form class="space-y-4 md:space-y-6" @submit.prevent="SignIn" method="post">
            <div>
              <label for="text" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">ชื่อผู้ใช้งาน <span
                  class="text-red-500">*</span></label>
              <input type="text" v-model="userLogin" id="userLogin" :class="[
              ...
              ]" placeholder="กรุณากรอกชื่อผู้ใช้งาน" @input="validateUserLogin" />
              <p v-if="userLoginError" class="mt-1 text-sm text-red-500">{{ userLoginError }}</p>
            </div>
            <div>
              <label for="password" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">รหัสผ่าน <span
                  class="text-red-500">*</span></label>
              <input type="password" v-model="passwordLogin" id="passwordLogin" placeholder="••••••••" :class="[
                'bg-gray-50 border text-gray-900 sm:text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500',
